...
async def user_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        db_users.delete_user(call.data[2:])
        markup = create_markup('inline', 1, ['❌ Назад', 'cancel'])
        photo = open('static/img/delete_user.png', 'rb')
        await bot.edit_message_media(types.InputMediaPhoto(photo, admin.text_delete_user), reply_markup=markup)
